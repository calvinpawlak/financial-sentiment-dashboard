...
# above for why this exists. Args: 0 = working directory, 1 = full command
# line to run. Re-written every time this script runs so it always matches
...
function Register-IngestionTask {
    ...
    # docstring above.
    $CommandLine = "`"$PythonPath`" $PythonArgument"
    $TaskArgument = "`"$VbsPath`" `"$ProjectDir`" `"$CommandLine`""
